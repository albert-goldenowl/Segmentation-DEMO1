...
class Camera(QThread):
    ...
    def run(self):
        ...
        while self.flag == True:
            ...
            if ret:
                sample_window = frame[y0:y0+target_shape[1],
                                      x0:x0+target_shape[0]].copy()
                ...
                y_pred = ort_session.run(
                    [output_name], {input_name: input_to_model})
                res = np.argmax(y_pred[0][0], axis=-1)
                mask_output = create_img(
                    res, self.square, self.circle, self.triangle, self.star)
                cv2.rectangle(frame, (x0, y0), (x1, y1), (0, 255, 0), 2)
                if self.background == -1:
                    pass
                else:
                    mask_output = create_img_bg(
                        sample_window, res, self.square, self.circle, self.triangle, self.star)
                frame[y0:y0+target_shape[1], x0:x0 +
                      target_shape[0]] = mask_output
